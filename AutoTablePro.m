...
line=1;
str(line)=strcat("井段",char(dataInterval(line)),"m，存在套管变形特征，变形长度为",num2str(data4(line,3)),...
    "m，最大变形点深度为",num2str(data4(line,4)),"m，测量最小内径为",num2str(data4(line,5)),"mm，测量平均内径为",...
    num2str(data4(line,6)),"mm，测量最大内径为",num2str(data4(line,7)),"mm，最大变形量为",num2str(data4(line,8)),...
    "mm，变形程度为",num2str(data4(line,9)),"%，根据解释标准评价为",char(dataInterpretation(line)),"。");
strFinal=str(1);
if textLineNum>1
